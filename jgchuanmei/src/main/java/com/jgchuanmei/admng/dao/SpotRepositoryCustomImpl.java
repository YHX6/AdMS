...
public class SpotRepositoryCustomImpl implements SpotRepositoryCustom {
    ...
    public List<Double> getPieData() {
        ...
        int d1 = (Integer) query.setParameter(1, "已占用").getResultList().get(0);
        int d2 = (Integer) query.setParameter(1, "未占用").getResultList().get(0);
        int d3 = (Integer) query.setParameter(1, "其他").getResultList().get(0);
...
//        // the number of spots of each station
//        List<BarChartPair> l2 = entityManager.createNativeQuery("select count(*) as count, station from spot where station!= '/' group by station", BarChartPair.class).getResultList();
//        // the number of ads(non droped) of each station
//        List<BarChartPair> l1 = entityManager.createNativeQuery("select count(*) as count,  spot.station as station from spot, ad  where spot.train ='/' and ad.spot_id = spot.id and ad.isdroped = 0 group by station", BarChartPair.class).getResultList();
//        return Arrays.asList(l1, l2);
//    }

//    @Override
//    public List<BarChartPair> getBarDataSpot() {
//        return entityManager.createNativeQuery("select count(*) as count, station from spot where station!= '/' group by station", BarChartPair.class).getResultList();
//    }
//
//    @Override
//    public List<BarChartPair> getBarDataAd() {
//        return entityManager.createNativeQuery("select count(*) as count,  spot.station as station from spot, ad  where spot.train ='/' and ad.spot_id = spot.id and ad.isdroped = 0 group by station", BarChartPair.class).getResultList();
//    }
